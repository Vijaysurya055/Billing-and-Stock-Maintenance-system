...
def logout():
    ...
    if sure == True:
        ...
        page1.entry1.delete(0, END)
        page1.entry2.delete(0, END)
# def print_bill(biller):
#     # Get the dimensions of the Toplevel window
#     width = biller.winfo_width()
#     height = biller.winfo_height()

#     # Capture the bill window as an image
#     hwnd = biller.winfo_id()
...
#     # Draw the captured image on the printer DC
...
#     bmp.SaveDC()
#     hdc.BitBlt((0, 0), (width, height), memdc, (0, 0), win32con.SRCCOPY)
...
class bill_window:
    def __init__(self, top=None):
        top.geometry("1366x768")
        top.resizable(0, 0)
        top.title("Jeyam Billing System")

        self.label = Label(biller)
        self.label.place(relx=0, rely=0, width=1366, height=768)
        self.img = PhotoImage(file="./images/bill_window4.png")
        ...
        self.button6.configure(text="""Print Bill""")
        self.button6.configure(command=self.print_bill)
    ...
    def total_bill(self):
        if self.cart.isEmpty():
            ...
            if strr.find('Total')==-1:
                ...
                gst = (18/100)*self.cart.total()
                g= round(gst, 2)
                gst = "\nGST\t\t\t\t\t\t\t\t\t\t\tRs. {}".format(g)
                total = "\nTotal\t\t\t\t\t\t\t\t\t\t\tRs. {}".format(self.cart.total()+g)
                self.Scrolledtext1.insert('insert', gst)
    ...
    def print_bill(self):
        try:
            hwnd = self.biller.winfo_id()
            rect = self.biller.winfo_geometry().split('+')[0]
